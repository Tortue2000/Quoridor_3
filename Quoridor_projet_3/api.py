...
def lister_parties(idul):
    rep = requests.get(f'{URL}parties/', params={'idul': idul})
...
def initialiser_partie(idul):
    rep = requests.post(f'{URL}partie/', data={'idul': idul})
    if rep.status_code == 200:
        rep = rep.json()
    elif rep.status_code == 406:
        rep = rep.json()
        ...
        print(f"Le POST sur '{URL}partie/' a produit le code d'erreur {rep.status_code}.")
...
def jouer_coup(id_partie, type_coup, position):
    rep = requests.put(f'{URL}jouer/',
    data={'id_partie': id_partie, 'type_coup': type_coup, 'position': position})
    if rep.status_code == 200:
        rep = rep.json()
    elif rep.status_code == 406:
        rep = rep.json()
        ...
        print(f"Le PUT sur '{URL}jouer/' a produit le code d'erreur {rep.status_code}.")
